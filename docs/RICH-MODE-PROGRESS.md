...
> **Historical record** — describes the original Rich Mode branch
> (`allele/session/09521f62-…`), which drove `claude -p --input-format
> stream-json` as a child process. That architecture was retired on
> 2026-04-21 and replaced with a read-only sidecar that tails Claude
> Code's JSONL transcript. Item 6 ("stream-json stdin") is no longer in
> the shipping code. See `docs/RICH-SIDECAR.md` for the current design.
