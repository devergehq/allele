...
impl StreamParser {
                ...
                if let (Some(event), Some(name)) = (sys.hook_event, sys.hook_name) {
                    match event.as_str() {
                        "PreToolUse" | "PostToolUse" | "Notification" | "Stop" => ParsedLine::new(
                            vec![RichEvent::HookStatus { hook_event: event, hook_name: name }],
                            Coverage::Full,
                        ),
    ...
    fn handle_assistant(&mut self, msg: AssistantMessage) -> ParsedLine {
        ...
        let coverage = if fell_back { Coverage::Partial } else { Coverage::Full };
        ParsedLine { events, coverage, diagnostics }
    }
        ...
        match wrapper.event {
            StreamEventInner::ContentBlockDelta { delta, .. } => match delta {
                Delta::Text { text } => ParsedLine::new(
                    vec![RichEvent::TextDelta { text, parent_agent_id: None }],
                    Coverage::Full,
                ),
                Delta::Thinking { thinking } => {
                    if !thinking.is_empty() {
                        ParsedLine::new(
                            vec![RichEvent::ThinkingBlock { thinking, parent_agent_id: None }],
                            Coverage::Full,
                        )
                    } else {
                        ParsedLine::new(Vec::new(), Coverage::Ignored)
...
mod tests {
    ...
    fn unrecognised_content_block_is_captured() {
        ...
        let has_fallback = parsed.events.iter().any(|e| matches!(
            e,
            RichEvent::Fallback { raw, .. } if raw.contains("redacted_thinking")
        ));
        assert!(has_fallback, "expected the unknown block preserved as Fallback");
    }
