...
impl SessionLedger {
    ...
    pub fn ingest(
        ...
        let ParsedLine { events, coverage, diagnostics } = parser.feed_line_detailed(raw_line);
...
mod tests {
    ...
    fn unsupported_lines_remain_inspectable() {
        ...
        assert!(matches!(unknown.events.first(), Some(RichEvent::Fallback { .. })));
        ...
        assert!(matches!(bad.events.first(), Some(RichEvent::Fallback { .. })));
    }
    ...
    fn coverage_stats_are_accurate() {
        ...
        assert_eq!(s.ignored, 1, "the rate-limit line emits nothing but is retained");
    ...
    fn historical_subagent_lines_are_ingested() {
        ...
        ledger.ingest(
            &mut parser,
            EventSource::Subagent { agent_id: "a1".into(), historical: true },
            line,
        );
