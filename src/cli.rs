...
mod tests {
    ...
    fn no_arguments_opens_the_app() {
        assert_eq!(classify::<&str>(&[]), Launch::Gui);
    }

    #[test]
    fn known_modes_are_allowed_through() {
        assert_eq!(classify(&["--mcp-serve"]), Launch::Gui);
        assert_eq!(classify(&["--capture-ui"]), Launch::Gui);
    }
    ...
    fn launch_services_process_serial_number_is_tolerated() {
        assert_eq!(classify(&["-psn_0_774221"]), Launch::Gui);
    }
    ...
    fn cocoa_defaults_overrides_consume_their_value() {
        ...
        assert_eq!(
            classify(&["-NSDocumentRevisionsDebugMode", "YES"]),
            Launch::Gui
        );
        assert_eq!(classify(&["-AppleLanguages", "(en)"]), Launch::Gui);
    }
