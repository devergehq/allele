...
impl ComposerCapabilities {
    ...
    pub const TEXT_ONLY: ComposerCapabilities = ComposerCapabilities {
...
mod tests {
    ...
    fn empty_draft_is_rejected() {
        let d = Draft::new();
        assert_eq!(
            d.validate(ComposerCapabilities::default()),
        ...
        ws.text = "   \n".into();
        assert_eq!(
            ws.validate(ComposerCapabilities::default()),
            Err(SubmitError::Empty)
        );
    }
    ...
    fn pending_attachment_blocks_until_ready() {
        ...
        assert_eq!(
            d.validate(ComposerCapabilities::default()),
            Err(SubmitError::AttachmentsNotReady)
        );
    }
    ...
    fn capabilities_gate_attachments_and_images() {
        ...
        assert_eq!(
            img.validate(ComposerCapabilities::TEXT_ONLY),
            Err(SubmitError::AttachmentsUnsupported)
        );
        // Attachments-yes, images-no agent rejects the image specifically.
        let caps = ComposerCapabilities {
            accepts_attachments: true,
            accepts_images: false,
        };
