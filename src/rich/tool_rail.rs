...
pub fn classify_tool(tool_name: &str) -> ToolClass {
    match tool_name {
        ...
        | "Bash" | "bash" | "NotebookRead" | "WebFetch" | "WebSearch" => ToolClass::Routine,
        // File mutations: prominent.
        "Write" | "write_file" | "Edit" | "edit_file" | "MultiEdit" | "NotebookEdit"
        | "Apply" | "apply_patch" => ToolClass::Mutation,
...
impl RoutineRailSummary {
    ...
    pub fn headline(&self, max_targets: usize) -> String {
        ...
        if self.searches > 0 {
            parts.push(format!("{} search{}", self.searches, if self.searches == 1 { "" } else { "es" }));
        }
        if self.shell > 0 {
...
mod tests {
    ...
    fn reads_and_searches_are_routine() {
        for name in ["Read", "read_file", "Grep", "Glob", "LS", "Bash", "WebFetch"] {
