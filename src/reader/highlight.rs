...
const KW_RUSTLIKE: &[&str] = &[
    ...
    "use", "where", "while", "class", "def", "function", "var", "new", "public", "private",
    "protected", "import", "from", "export", "default", "interface", "extends", "implements",
    "package", "func", "go", "defer", "chan", "map", "range", "nil", "null", "None", "True", "False",
    "and", "or", "not", "is", "lambda", "try", "except", "finally", "raise", "with", "yield",
    "throw", "catch", "switch", "case", "typeof", "instanceof", "void", "int", "string", "bool",
    "float", "double", "char", "long", "short", "unsigned", "signed", "template", "typename",
    "namespace", "using", "override", "virtual", "abstract", "final", "let", "const", "elif",
];
        ...
        while i < chars.len() {
            ...
            if syntax.quotes.contains(&c) {
                ...
                while i < chars.len() {
                    ...
                    if ch == quote {
                        i += 1;
                        end_byte = if i < chars.len() { chars[i].0 } else { line.len() };
                        break;
                    }
                    i += 1;
                    end_byte = if i < chars.len() { chars[i].0 } else { line.len() };
            ...
            if c.is_ascii_digit() {
                ...
                let end = if i < chars.len() { chars[i].0 } else { line.len() };
