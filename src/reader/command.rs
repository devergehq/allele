...
impl AppState {
    pub(crate) fn open_command_palette(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.command_palette_input.focus_handle(cx).focus(window, cx);
        cx.notify();
    }
    ...
    fn recompute_commands(&mut self) {
        let Some(query) = self.command_palette.as_ref().map(|p| p.query.trim().to_lowercase()) else {
            return;
        };
    ...
    fn build_commands(&self) -> Vec<CommandItem> {
        ...
        // ── Surfaces ──────────────────────────────────────────────
        push("Go to Claude", Some("Main tab"), "surface tab agent", None, false, None, Cmd::Tab(MainTab::Claude));
        push("Go to Reader", Some("Files & source"), "surface tab files source read", None, false, None, Cmd::Tab(MainTab::Reader));
        push("Go to Transcript", Some("Session transcript"), "surface tab log history", Some("⌘⇧R"), false, None, Cmd::Tab(MainTab::Transcript));
        let browser_disabled = if self.browser_tab_available() {
            None
        } else {
            Some("No linked Chrome tab for this session".to_string())
        };
        push("Go to Browser", Some("Linked Chrome tab"), "surface tab web chrome", None, false, browser_disabled, Cmd::Tab(MainTab::Browser));

        // ── Panels & overlays ─────────────────────────────────────
        push("Toggle Sidebar", None, "panel projects sessions hide show", Some("⌘B"), false, None, Cmd::ToggleSidebar);
        push("Toggle Changes Panel", None, "panel git diff review", None, false, None, Cmd::ToggleChanges);
        push("Go to File…", Some("Fuzzy file search"), "open find path cmd-p", Some("⌘P"), false, None, Cmd::GoToFile);
        push("Search Project…", Some("Content & symbols"), "grep find text symbol", Some("⌘⇧F"), false, None, Cmd::SearchProject);
        push("Open Scratch Pad", None, "notes compose", Some("⌘K"), false, None, Cmd::OpenScratchPad);
        ...
        push("New Session…", Some("In the current project"), "create add worktree", None, false, new_disabled, Cmd::NewSession);
        ...
        for (p_idx, project) in self.projects.iter().enumerate() {
            for (s_idx, session) in project.sessions.iter().enumerate() {
                ...
                cmds.push(CommandItem {
                    ...
                    keywords: format!("switch go project session {} {}", project.name, session.label),
                    shortcut: None,
                    destructive: false,
                    disabled: None,
    ...
    pub(crate) fn render_command_palette(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        for (row, &cmd_idx) in palette.results.iter().enumerate() {
            ...
            let sub_color = if disabled { theme().warning } else { theme().text_ghost };
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .flex_col()
                            .gap(px(1.0))
                            .child(div().text_size(px(12.0)).text_color(title_color).child(item.title.clone()))
                            .when_some(sub, |d, s| {
