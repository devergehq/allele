    ...
    pub(crate) fn render_search(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mode_btn = |mode: SearchMode, active: bool| {
            div()
                ...
                .cursor_pointer()
                .when(active, |d| {
                    d.bg(theme().bg_active).text_color(theme().text_primary)
                })
                .when(!active, |d| {
                    d.text_color(theme().text_faint)
        ...
        let panel = div()
            ...
            .child(
                div()
                    ...
                    .child(mode_btn(
                        SearchMode::Content,
                        state.mode == SearchMode::Content,
                    ))
                    .child(mode_btn(
                        SearchMode::Filename,
                        state.mode == SearchMode::Filename,
                    ))
                    .child(mode_btn(
                        SearchMode::Symbol,
        ...
        root = root.child(deferred(
            div()
                ...
                .on_key_down(
                    cx.listener(|this: &mut Self, event: &KeyDownEvent, _w, cx| {
...
mod tests {
    ...
    fn symbol_name_handles_modifiers_and_keywords() {
        ...
        assert_eq!(
            symbol_name("    pub(crate) struct FileIndex {"),
            Some("FileIndex")
        );
        assert_eq!(
            symbol_name("export default class App extends X {"),
            Some("App")
        );
