...
impl AppState {
    ...
    /// Delegates to a per-family handler so each match only considers
    /// its own sub-enum. `skip_refocus` is a shared out-param — a
    /// handler sets it to suppress the post-dispatch re-focus-active-
    /// terminal behaviour (drawer interactions manage focus themselves).
    pub(crate) fn dispatch_pending_action(
        ...
        match action {
            PendingAction::Session(a)  => self.handle_session_action(a, window, cx, &mut skip_refocus),
            PendingAction::Archive(a)  => self.handle_archive_action(a, window, cx, &mut skip_refocus),
            PendingAction::Drawer(a)   => self.handle_drawer_action(a, window, cx, &mut skip_refocus),
            PendingAction::Sidebar(a)  => self.handle_sidebar_action(a, window, cx, &mut skip_refocus),
            PendingAction::Project(a)  => self.handle_project_action(a, window, cx, &mut skip_refocus),
            PendingAction::Settings(a) => self.handle_settings_action(a, window, cx, &mut skip_refocus),
            PendingAction::Browser(a)  => self.handle_browser_action(a, window, cx, &mut skip_refocus),
            PendingAction::Overlay(a)  => self.handle_overlay_action(a, window, cx, &mut skip_refocus),
        }

        // After any sidebar-triggered action, re-focus the active
        // terminal so keyboard input goes back to Claude Code.
        // Drawer / settings / overlay actions manage their own focus
        // and set skip_refocus=true.
        if !skip_refocus {
    ...
    fn handle_session_action(
        ...
        match action {
            ...
            SessionAction::AddToProject(project_idx) => {
                self.add_session_to_project(project_idx, window, cx);
            }
            SessionAction::CloseKeepClone { project_idx, session_idx } => {
            ...
            SessionAction::RequestDiscard { project_idx, session_idx } => {
            ...
            SessionAction::Discard { project_idx, session_idx } => {
                self.confirmations.discard = None;
                self.remove_session(
                    SessionCursor { project_idx, session_idx },
                    window,
            ...
            SessionAction::Select { project_idx, session_idx } => {
                let cursor = SessionCursor { project_idx, session_idx };
    ...
    fn handle_project_action(
        ...
        match action {
            ...
            ProjectAction::Remove(project_idx) => {
                self.remove_project(project_idx, window, cx);
            }
            ProjectAction::Relocate(project_idx) => {
    ...
    fn handle_archive_action(
        ...
        match action {
            ArchiveAction::Merge { project_idx, archive_idx } => {
            ...
            ArchiveAction::Delete { project_idx, archive_idx } => {
                if let Some(project) = self.projects.get_mut(project_idx) {
                    if let Some(entry) = project.archives.get(archive_idx) {
                        let session_id = entry.id.clone();
                        let _ = git::delete_ref(
    ...
    fn handle_drawer_action(
        ...
        match action {
            ...
            DrawerAction::StartRenameTab(idx) => {
                *skip_refocus = true;
                if let Some(cursor) = self.active {
    ...
    fn handle_sidebar_action(
        ...
        match action {
            SidebarAction::ToggleLeft => {
                self.sidebar.visible = !self.sidebar.visible;
                self.mark_settings_dirty();
            }
            SidebarAction::ToggleRight => {
    ...
    fn handle_browser_action(
        ...
        match action {
            BrowserAction::SyncToActive => {
                *skip_refocus = true;
                self.sync_browser_to_active();
            }
            BrowserAction::CloseForSession { project_idx, session_idx } => {
                *skip_refocus = true;
                let cursor = SessionCursor { project_idx, session_idx };
                let tab_id = self
                    .projects
